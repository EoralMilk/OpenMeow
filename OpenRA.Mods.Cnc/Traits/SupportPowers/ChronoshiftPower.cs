	...
	class ChronoshiftPower : SupportPower
	{
		...
		class SelectChronoshiftTarget : OrderGenerator
		{
			...
			protected override IEnumerable<IRenderable> Render(WorldRenderer wr, World world)
			{
				...
				foreach (var t in tiles)
					yield return new SpriteRenderable(tile, wr.World.Map.CenterOfCell(t), WVec.Zero, -511, palette, 1f, alpha, float3.Ones, TintModifiers.IgnoreWorldTint, true, SpriteMeshType.Plane);
			}
		...
		class SelectDestination : OrderGenerator
		{
			...
			protected override IEnumerable<IRenderable> RenderAboveShroud(WorldRenderer wr, World world)
			{
				...
				foreach (var t in power.CellsMatching(sourceLocation, footprint, dimensions))
				{
					...
					yield return new SpriteRenderable(tile, wr.World.Map.CenterOfCell(t + delta), WVec.Zero, -511, palette, 1f, alpha, float3.Ones, TintModifiers.IgnoreWorldTint, true, SpriteMeshType.Plane);
				}
				...
				foreach (var unit in power.UnitsInRange(sourceLocation))
				{
					if (unit.CanBeViewedByPlayer(manager.Self.Owner))
					{
						...
						yield return new SpriteRenderable(tile, wr.World.Map.CenterOfCell(targetCell), WVec.Zero, -511, palette, 1f, alpha, float3.Ones, TintModifiers.IgnoreWorldTint, true, SpriteMeshType.Plane);
					}
			...
			protected override IEnumerable<IRenderable> Render(WorldRenderer wr, World world)
			{
				...
				// Source tiles
				foreach (var t in power.CellsMatching(sourceLocation, footprint, dimensions))
					yield return new SpriteRenderable(sourceTile, wr.World.Map.CenterOfCell(t), WVec.Zero, -511, palette, 1f, sourceAlpha, float3.Ones, TintModifiers.IgnoreWorldTint, true, SpriteMeshType.Plane);
			}
