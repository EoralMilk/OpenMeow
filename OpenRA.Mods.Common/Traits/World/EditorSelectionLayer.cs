			...
			if (CopyRegion != null)
				foreach (var c in CopyRegion)
					yield return new SpriteRenderable(copyTile, wr.World.Map.CenterOfCell(c),
							WVec.Zero, -511, palette, 1f, copyAlpha * info.FootprintAlpha, float3.Ones, TintModifiers.IgnoreWorldTint, true, SpriteMeshType.Plane);

			if (PasteRegion != null)
				foreach (var c in PasteRegion)
					yield return new SpriteRenderable(pasteTile, wr.World.Map.CenterOfCell(c),
						WVec.Zero, -511, palette, 1f, pasteAlpha * info.FootprintAlpha, float3.Ones, TintModifiers.IgnoreWorldTint, true, SpriteMeshType.Plane);
		}
